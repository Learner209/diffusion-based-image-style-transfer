...
def make_tranforms(image_transforms):
    ...
    #     image_transforms = [instantiate_from_config(tt) for tt in image_transforms]
    image_transforms = []
    image_transforms.extend([transforms.Resize((512, 768), interpolation=3),
                             transforms.ToTensor(),
                             transforms.Lambda(lambda x: rearrange(x * 2. - 1., 'c h w -> h w c'))])
    image_transforms = transforms.Compose(image_transforms)
...
class FolderData(Dataset):
    def __init__(self,
        ...
        print('Using image transforms: ', image_transforms)
        self.tform = make_tranforms(image_transforms)
    ...
    def process_im(self, im):
        # im = im.convert("RGB")
        assert isinstance(im, Image.Image), f"Didn't get an image: {type(im)}"
...
if __name__ == '__main__':
    # dataset = hf_dataset(name='lambdalabs/pokemon-blip-captions', split="train")
    # sample = dataset[0]
    # print(sample)
    # print(sample['txt'])

    dataset = FolderData(root_dir="/mnt/homes/minghao/AI/final_project/datasets/photo_mural_data/mural", ext="png")
    sample = dataset[0]
    print("The length of the sample:", len(dataset))
    ...
    for epoch in tqdm(range(max_epochs), desc="Training epoch..."):
        for idx, local_batch in enumerate(data_loader):
            if idx == 10:
                break
            local_batch = local_batch['image'].to("cuda:1")
            print(idx)
